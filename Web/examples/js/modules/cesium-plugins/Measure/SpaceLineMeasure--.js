function SpaceMeasureTool(viewer) {
  this.viewer = viewer;
  this.handler = null;
  this.line = null;
  ...
  //测量空间直线距离 
  /******************************************* */
  SpaceMeasureTool.prototype.measureLineSpace = function () {
    // 取消双击事件-追踪该位置
    ...
    this.handler.setInputAction(function (movement) {
      ...
      that.floatingPointArray.push(floatingPoint);
    ...
    var PolyLinePrimitive = (function () {
      ...
      _.prototype._init = function () {
        ...
        that.line = that.viewer.entities.add(this.options);
  ...
  SpaceMeasureTool.prototype.clean = function () {
    if (this.line) {

      this.viewer.entities.remove(this.line)
    }
    if (this.floatingPointArray) {

      for (let i = 0; i < this.floatingPointArray.length; i++) {
        this.viewer.entities.remove(this.floatingPointArray[i])
      }
    }
  ...
  //****************************测量空间面积************************************************//
  SpaceMeasureTool.prototype.measureAreaSpace = function () {
    // 取消双击事件-追踪该位置
    this.viewer.cesiumWidget.screenSpaceEventHandler.removeInputAction(Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
    // 鼠标事件
    this.handler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene._imageryLayerCollection);
    ...
    this.handler.setInputAction(function (movement) {
      ...
      if (positions.length >= 2) {
        if (!Cesium.defined(this.polygon)) {
          this.polygon = new PolygonPrimitive(positions);
        } else {
          positions.pop();
          // cartesian.y += (1 + Math.random());
          positions.push(cartesian);
    ...
    this.handler.setInputAction(function (movement) {
      ...
      var stopPoint=that.viewer.entities.add({
        name: '多边形面积',
        position: positions[positions.length - 1],
        // point : {
        //  pixelSize : 5,
      ...
      that.floatingPointArray.push(stopPoint)
    }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
    ...
    var PolygonPrimitive = (function () {
      ...
      _.prototype._init = function () {
       ...
       that.polygon= that.viewer.entities.add(this.options);
      };
