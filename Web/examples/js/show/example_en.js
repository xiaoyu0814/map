...
function initPage() {
    ...
    $("#sidebar-toggle").on("click",function(e){
        T = !T;
        var $titleBar = $(".sidebar-title-bar");
        if(T){
            console.log($titleBar)
            for (var i = 0; i < $titleBar.length; i++) {
                $titleBar.eq(i).removeClass("bag");
            }
        }else{
...
function createGalleryItem(id, config) {
    ...
    var title = utils.getLocalPairs(config, "name_en");
    if (title) {
...
function createSubGalleryItem(config, name) {
    ...
    for (var key in config) {
        ...
        var title = utils.getLocalPairs(configItem, "name_en");
...
function createGalleryChart(example) {
    var target = "editor.html",
        defaultThumb = "./image/thumb.png",
        title = utils.getLocalPairs(example, "name_en"),
